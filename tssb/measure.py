...
def create_gaussian_trm(eta, cov, prior, a, l, gamma):
    ...
    tssb = TSSB(alpha=depth_weight(a, l), gamma=gamma)
    return TreeRandomMeasure(tssb, initial, next)

if __name__ == "__main__":
    from util import ScipySampler
    t = create_gaussian_trm(1, np.eye(2)/1000.0, ScipySampler(stats.multivariate_normal([0, 0], np.eye(2))), 25, 1, 1)
